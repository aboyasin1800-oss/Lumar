...
public interface ILoyaltyRepository
{
    ...
    Task<IReadOnlyList<LoyaltyAccountDto>> GetAllAccountsAsync(CancellationToken cancellationToken);
    Task<LoyaltyAccountDto?> UpdateAccountLifecycleAsync(int customerId, string status, DateTime? warningStartedAtUtc, DateTime? frozenAtUtc, DateTime? reactivatedAtUtc, string? freezeReason, DateTime? lastQualifyingActivityAtUtc, CancellationToken cancellationToken);
    ...
    Task<LoyaltyTransactionResultDto> CreateTransactionAsync(int customerId, string transactionType, decimal points, int? orderId, int? rewardId, string source, string? notes, CancellationToken cancellationToken, bool allowFrozenQualifyingPurchase = false);
}
