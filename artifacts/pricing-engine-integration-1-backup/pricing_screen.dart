...
class _PricingScreenState extends State<PricingScreen> {
	...
	Future<List<_PieceCostSetting>> _load() async {
		final response = await http.get(Uri.parse('$_baseUrl/piece-cost-management'));
		...
		builder: (context, snapshot) {
			...
			return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
				Row(children: [
					...
					FilledButton.icon(
						onPressed: () => AppNavigation.push(
							context,
							(_) => const PieceCostManagementScreen(),
						),
				...
				Expanded(child: items.isEmpty ? const Center(child: Text('لا توجد أنواع قطع مطابقة.')) : LayoutBuilder(builder: (context, constraints) {
					if (constraints.maxWidth < 950) return ListView.separated(itemCount: items.length, separatorBuilder: (_, __) => const SizedBox(height: 8), itemBuilder: (context, index) => _PieceCostTile(item: items[index]));
					return _PieceCostTable(items: items);
				})),
			]);
		},
	);
...
class _PieceCostTable extends StatelessWidget {
	const _PieceCostTable({required this.items});
...
class _PieceCostTile extends StatelessWidget {
	const _PieceCostTile({required this.item});
	final _PieceCostSetting item;
...
class _PieceCostSetting {
	...
	factory _PieceCostSetting.fromJson(Map<String, dynamic> json) => _PieceCostSetting(
		...
		totalOperationalCost: (json['totalOperationalCost'] as num?)?.toDouble() ??
				((json['sewingCost'] as num?)?.toDouble() ?? 0) +
		...
		isConfigured: json['isConfigured'] == true ||
				((json['sewingCost'] as num?)?.toDouble() ?? 0) != 0 ||
				((json['consumablesCost'] as num?)?.toDouble() ?? 0) != 0 ||
				((json['ironingAndPackagingCost'] as num?)?.toDouble() ?? 0) != 0 ||
				((json['fixedOperatingCost'] as num?)?.toDouble() ?? 0) != 0,
	);
